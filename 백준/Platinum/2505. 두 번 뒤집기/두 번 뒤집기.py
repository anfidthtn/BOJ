...
def sectionClose(sections, endIdx):
    # 나중에 복사할 때 주소복사 아닌 값 복사 해야해서 이렇게
    sections[-1]['nums'] = tuple(sections[-1]['nums'])
...
def sectionOpen(sections, startIdx):
    sections.append(
        {'nums': [nums[startIdx], nums[startIdx]], 'points': [startIdx, NO]})
...
def mergeSection(targetSection, theOtherSection):
    ...
    merge_points = (targetSection['points'][START],
                    theOtherSection['points'][END])
...
def setSectionPoint(prevSection, nowSection):
    ...
    nowSection['points'] = (prevSection['points'][END] + 1,
                            prevSection['points'][END] + 1 + now_diff)
...
def checkSectionMergeAble(targetSection, theOtherSection):
    ...
    # 앞 구간 끝지점, 뒷구간 시작점 맞는 경우 (사실 여기 들어온다면 무조건인데 혹시 몰라서 체크)
    if targetSection['points'][END] + 1 == theOtherSection['points'][START]:
        sectionDiff = targetSection['nums'][END] - \
            theOtherSection['nums'][START]  # 앞뒤 섹션간 숫자 차이
        if targetSection['nums'][START] == targetSection['nums'][END] or theOtherSection['points'][START] == theOtherSection['points'][END]:
            ...
            if abs(sectionDiff) > 1:
                ...
                return False
            # 둘의 차이가 1이거나 -1인 경우
            targetDiff = targetSection['points'][START] - \
                targetSection['points'][END]
            theOtherDiff = theOtherSection['points'][START] - \
                theOtherSection['points'][END]
            # 둘이 부호가 같은 경우 (내림차순이든 오름차순이든 같은 경우)
            if targetDiff * theOtherDiff > 0:
...
def getNextInfos(nowInfo, rStartSectionIdx, rEndSectionIdx):
    ...
    # 구간 단위로 리버스 하는 경우
    tempSections = []
    # 얘는 뒤집을 거 아니니 얕은복사해도 됨(값을 보기만 할 예정)
    tempSections.append(nowSections[rStartSectionIdx - 1])
    # j번째부터 i번째까지 내려가면서 뒤집어넣음
    for reverseTempIdx in range(rEndSectionIdx, rStartSectionIdx - 1, -1):
        # 얘는 뒤집어야해서 깊은 복사
        reversedTempSection = nowSections[reverseTempIdx].copy()
        sectionNumReverse(reversedTempSection)  # 색션의 시작넘버, 끝 넘버를 뒤집음
        tempSections.append(reversedTempSection)
    # 얘는 뒤집을 거 아니니 얕은복사해도 됨(값을 보기만 할 예정)
    tempSections.append(nowSections[rEndSectionIdx + 1])

    # 지금 tempSections에는 넘버만 뒤집힌 상태로 애들이 있음
    newSections = []  # tempSection에서 START, END point를 재설정하고, 그에 따른 합치기 등을 함
    # i ~ j번째까지 회전시켰으니 좌측 0 ~ (i - 1)번째까지는 그대로
    for leftNonReverseIdx in range(0, rStartSectionIdx):
        # (당장 여기서 뿐만 아니라 뒤에도)값이 바뀔 수 있으니 깊은 복사 해줘야함
        newSections.append(nowSections[leftNonReverseIdx].copy())
    # 1 ~ 뒤집은 구간 개수 만큼 봐야함
    for reverseIdx in range(1, rEndSectionIdx - rStartSectionIdx + 3):
    ...
    # 나머지(위의 예시에서 원본의 인덱스 8 ~ 9)는 그대로 뒤에 붙여줌
    for rightNonReverseIdx in range(rEndSectionIdx + 2, len(nowSections)):
        newSections.append(nowSections[rightNonReverseIdx].copy())  # 깊은 복사

    # 위의 처리(돌리기) 이후
    # 더 돌릴 횟수가 0번 남으면(nowInfo[1] == 1상태) 1구간(완전히 오름차순 정렬 완료)만 있어야하고
    # 더 돌릴 횟수가 1번 남으면(nowInfo[1] == 0상태) 최대 3구간 (가운데 구간을 1번 돌려서 0번남기며 1구간 남기기로 전환)까지여야 함
    # 더 돌릴 횟수에 따라 기준 구간수보다 더 많은 구간이 나눠진 상태라면 더이상 돌려도 남은 횟수 안에 복구가 불가능해서 early stop
    if len(newSections) <= 3 - (nowInfo[1] * 2):
        nextHistory = nowInfo[2].copy()
        # 현재 색션에서 Start의 시작점 - End의 끝점을 돌렸으므로, 해당 지점을 history에 넣어준다.
        nextHistory.append((nowSections[rStartSectionIdx]['points']
                           [START], nowSections[rEndSectionIdx]['points'][END]))
    ...
    if rEndSectionIdx - rStartSectionIdx == 1 and abs(nowSections[rStartSectionIdx]['nums'][START] - nowSections[rEndSectionIdx]['nums'][END]) == 1:
        if getLenSection(nowSections[rStartSectionIdx]) == 2 and getLenSection(nowSections[rEndSectionIdx]) > 2:
            ...
            splitSection = {'nums': (nowSections[rStartSectionIdx]['nums'][START],
                                     nowSections[rEndSectionIdx]['nums'][START]), 'points': (NO, NO)}
            setSectionPoint(tempSections[-1], splitSection)
            tempSections.append(splitSection)
            splitSection = {'nums': (
                nowSections[rStartSectionIdx]['nums'][END], nowSections[rStartSectionIdx]['nums'][END]), 'points': (NO, NO)}
            setSectionPoint(tempSections[-1], splitSection)
            tempSections.append(splitSection)
            tempSectionsInfoList.append(
                [tempSections, (nowSections[rStartSectionIdx]['points'][END], nowSections[rEndSectionIdx]['points'][END])])

    if rEndSectionIdx - rStartSectionIdx == 1 and abs(nowSections[rStartSectionIdx]['nums'][START] - nowSections[rEndSectionIdx]['nums'][END]) == 1:
        if getLenSection(nowSections[rStartSectionIdx]) >= 2 and getLenSection(nowSections[rEndSectionIdx]) == 2:
            tempSections = []
            tempSections.append(nowSections[rStartSectionIdx - 1].copy())
            splitSection = {'nums': (nowSections[rEndSectionIdx]['nums'][START],
                                     nowSections[rEndSectionIdx]['nums'][START]), 'points': (NO, NO)}
            setSectionPoint(tempSections[-1], splitSection)
            tempSections.append(splitSection)
            splitSection = {'nums': (
                nowSections[rStartSectionIdx]['nums'][END], nowSections[rEndSectionIdx]['nums'][END]), 'points': (NO, NO)}
            setSectionPoint(tempSections[-1], splitSection)
            tempSections.append(splitSection)
            tempSectionsInfoList.append(
                [tempSections, (nowSections[rStartSectionIdx]['points'][START], nowSections[rEndSectionIdx]['points'][START])])
    ...
    for tempSections in tempSectionsInfoList:
        ...
        for reverseIdx in range(1, len(tempSections)):  # 1 ~ 뒤집은 구간 개수 만큼 봐야함
            ...
            if checkSectionMergeAble(newSections[-1], reversedSection) is False:
                newSections.append(reversedSection)
        # 나머지(위의 예시에서 원본의 인덱스 8 ~ 9)는 그대로 뒤에 붙여줌
        for rightNonReverseIdx in range(rEndSectionIdx + 1, len(nowSections)):
            tempSection = nowSections[rightNonReverseIdx].copy()
            if checkSectionMergeAble(newSections[-1], tempSection) is False:
                newSections.append(tempSection)
        ...
        if len(newSections) <= 3 - (nowInfo[1] * 2):
            ...
            nextInfoList.append(
                [newSections, nowInfo[1] + 1, nextHistory])
...
for i in range(1, n + 2):
    if sections[-1]['points'][END] == NO:  # 이전 구간이 끝나지 않은 경우
        if abs(sections[-1]['nums'][END] - nums[i]) >= 2:  # 이전 구간 마지막 수 차이 2 이상이면 이전 구간 끝
            ...
            # 이 때는 차이가 -1이거나 1인 경우만이므로 그냥 그대로 등록
            sectionAddComponent(sections, i)
            ...
            # == -1상황 오름차순으로 들어오면 앞 구간 닫고 새로 등록
            elif sections[-1]['nums'][END] - nums[i] == -1:
...
while len(waiting) > 0:
    ...
    if len(nowSections) == 1:
        ...
        for i in range(2 - len(nowInfo[2])):
            print('1 1')
        exit()
