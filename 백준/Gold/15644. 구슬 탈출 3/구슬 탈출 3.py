...
for dir in direction:
    waiting.append({'R' : Rstart.copy(), 'B' : Bstart.copy(), 'Depth' : 1, 'dirSym' : dir, 'trace' : [dir]})
...
while len(waiting) > 0:
    ...
    if Bgoal is True:
        ...
        for dir in direction:
            trace = nowTry['trace'].copy()
            trace.append(dir)
            waiting.append({'R' : nowTry[R].copy(), 'B' : nowTry[B].copy(), 'Depth' : nowTry[Depth] + 1, 'dirSym' : dir, 'trace' : trace})
    else:
        print(nowTry[Depth])
        for dir in nowTry['trace']:
            if dir == (-1, 0):
                dir = 'U'
            elif dir == (1, 0):
                dir = 'D'
